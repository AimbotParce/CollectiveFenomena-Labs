...
! Use: P3-exercici-1.f90 -s <width> <height> -n <iteration_count> -t <temperature>
...
program main
    ...
    write(*, *) "Generating a random system with S = +1 or -1"
    ...
    call metropolis(S, width, height, PBCx, PBCy, E, T, Niter, fileName)
    call cpu_time(timeEnd)

    write(*, "(a, f10.4, a)") "Done! Time elapsed: ", timeEnd - timeStart, " seconds"
    if (makeGraphs) then
        write(*, *) "Generating plot of the convergence"
        call system('gnuplot -e "file='//"'"//'dat/'//fileName//"'"//'" gnu/plot_convergence.gnu')
    end if
    ...
    do while ( i <= num_args )
        ...
        if (args(i)(1:1) == '-') then
            ...
            ! Check if the argument is -n
                ...
                i = i + 1 ! Skip the next argument
            ! Check if the argument is -o
            else if (args(i)(2:2) == 'o') then
                fileName = args(i+1)
                i = i + 1 ! Skip the next argument
...
subroutine metropolis(mat, width, height, PBCx, PBCy, energy, temp, Niter, fileName)
    ...
    ! Open the file
    open(unit=10, file="dat/" // fileName, iostat=ios)
    ...
    write(*, *) "Data written to dat/" // fileName
